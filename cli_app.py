...
def main():
    ...
    auth_group.add_argument('--auth-method', choices=['access-key', 'profile', 'sso', 'role', 'environment', 'default'],
                           default='default', help='Authentication method (default: default)')
    
    # Access key authentication
    parser.add_argument('--access-key', help='AWS Access Key ID (for access-key method)')
    parser.add_argument('--secret-key', help='AWS Secret Access Key (for access-key method)')
    parser.add_argument('--session-token', help='AWS Session Token (optional, for access-key method)')
    ...
    parser.add_argument('--role-session-name', help='Role session name (optional, for role method)')
    parser.add_argument('--external-id', help='External ID (optional, for role method)')
    parser.add_argument('--mfa-serial', help='MFA device serial number (optional, for role method)')
    parser.add_argument('--mfa-token', help='MFA token (optional, for role method)')
    
    # Common settings
    parser.add_argument('--region', default='us-east-1', help='AWS Region (default: us-east-1)')
    parser.add_argument('--kms-key', default='alias/aws/ssm', help='Default KMS key alias')
    ...
    upload_parser.add_argument('--overwrite', action='store_true', help='Overwrite existing parameters without asking')
    
    # List command
    list_parser = subparsers.add_parser('list', help='List all parameters')
    list_parser.add_argument('--decrypt', action='store_true', default=True, help='Decrypt SecureString values')
    ...
    get_parser.add_argument('--decrypt', action='store_true', default=True, help='Decrypt if SecureString')
    ...
    list_profiles_parser = subparsers.add_parser('list-profiles', help='List available AWS profiles')
    ...
    if not manager.connect():
        print("Failed to connect to AWS. Please check your credentials and permissions.")
        return 1
    
    auth_info = aws_config.get_auth_info()
    print(f"Connected to AWS using {auth_info['method']} method in region: {auth_info['region']}")
...
def configure_aws_auth(args):
    ...
    if auth_method == 'access-key':
        if not args.access_key or not args.secret_key:
            raise ValueError("Access key and secret key are required for access-key authentication method")
        ...
        if not args.profile:
            raise ValueError("Profile name is required for profile authentication method")
        ...
        if not all([args.sso_start_url, args.sso_region, args.sso_account_id, args.sso_role_name]):
            raise ValueError("All SSO parameters are required for SSO authentication method")
    ...
    try:
        ...
        for param in parameters:
            ...
            if exists and not args.overwrite:
                ...
                print(f"  Current: {existing_param['Value'][:50]}{'...' if len(existing_param['Value']) > 50 else ''}")
                print(f"  New: {param['value'][:50]}{'...' if len(param['value']) > 50 else ''}")
...
def list_command(manager, args):
    ...
    try:
        ...
        if args.filter:
            parameters = [p for p in parameters if args.filter.lower() in p['Name'].lower()]
        ...
        if args.output:
            ...
            for param in parameters:
                ...
                value = param['Value'][:27] if len(param['Value']) > 27 else param['Value']
...
def delete_command(manager, args):
    ...
    try:
        ...
        if not args.force:
            ...
            print(f"Current value: {param['Value'][:100]}{'...' if len(param['Value']) > 100 else ''}")
            response = input("Are you sure you want to delete this parameter? (y/N): ").lower().strip()
            if response != 'y':
