...
class AWSConfig:
    ...
    def set_role_auth(
        ...
        self.role_session_name = (
            role_session_name or f"aws-parameter-store-{os.getpid()}"
        )
    ...
    def is_configured(self) -> bool:
        ...
        if self.auth_method == AuthMethod.ACCESS_KEY:
            ...
            return bool(
                self.sso_start_url
                and self.sso_region
                and self.sso_account_id
                and self.sso_role_name
            )
    ...
    def _create_sso_session(self) -> boto3.Session:
        ...
        try:
            ...
            raise ValueError(
                f"SSO authentication failed. Please ensure you've configured SSO via AWS CLI: {str(e)}"
            )
    ...
    def get_auth_info(self) -> Dict[str, Any]:
        ...
        info = {"method": self.auth_method.value, "region": self.region}

        if self.auth_method == AuthMethod.ACCESS_KEY:
            info["access_key_id"] = (
                self.access_key_id[:8] + "..." if self.access_key_id else None
            )
