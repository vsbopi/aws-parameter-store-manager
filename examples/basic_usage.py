...
def example_profile_auth():
    ...
    aws_config.set_profile_auth(
        profile_name="default",  # Replace with your profile name
        region="us-east-1"
    )
...
def example_environment_auth():
    ...
    if manager.connect():
        ...
        print("❌ Failed to connect (make sure AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY are set)")
...
def example_csv_operations():
    ...
    if manager.connect():
        ...
        try:
            csv_file = os.path.join(os.path.dirname(__file__), '..', 'sample_parameters.csv')
