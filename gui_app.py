...
class ParameterStoreGUI:
    ...
    def setup_config_tab(self):
        ...
        auth_method_frame = ttk.LabelFrame(scrollable_frame, text="Authentication Method", padding=10)
        ...
        for i, (value, text) in enumerate(auth_methods):
            rb = ttk.Radiobutton(auth_method_frame, text=text, variable=self.auth_method_var, 
                               value=value, command=self.on_auth_method_change)
            rb.grid(row=i//2, column=i%2, sticky=tk.W, padx=10, pady=2)
        
        # Common settings frame
        common_frame = ttk.LabelFrame(scrollable_frame, text="Common Settings", padding=10)
        common_frame.pack(fill=tk.X, padx=10, pady=5)
        
        # Region
        ttk.Label(common_frame, text="AWS Region:").grid(row=0, column=0, sticky=tk.W, pady=2)
        self.region_var = tk.StringVar(value="us-east-1")
        region_combo = ttk.Combobox(common_frame, textvariable=self.region_var, width=47)
        region_combo['values'] = ['us-east-1', 'us-east-2', 'us-west-1', 'us-west-2', 
                                 'eu-west-1', 'eu-west-2', 'eu-central-1', 'ap-southeast-1', 
                                 'ap-southeast-2', 'ap-northeast-1', 'ap-south-1', 'ca-central-1']
        region_combo.grid(row=0, column=1, sticky=tk.W, padx=(10, 0), pady=2)
        
        # KMS Key
        ttk.Label(common_frame, text="Default KMS Key:").grid(row=1, column=0, sticky=tk.W, pady=2)
        ...
        self.access_key_frame = ttk.LabelFrame(scrollable_frame, text="Access Key Configuration", padding=10)
        self.access_key_frame.pack(fill=tk.X, padx=10, pady=5)
        
        ttk.Label(self.access_key_frame, text="AWS Access Key ID:").grid(row=0, column=0, sticky=tk.W, pady=2)
        self.access_key_var = tk.StringVar()
        access_key_entry = ttk.Entry(self.access_key_frame, textvariable=self.access_key_var, width=50)
        access_key_entry.grid(row=0, column=1, sticky=tk.W, padx=(10, 0), pady=2)
        
        ttk.Label(self.access_key_frame, text="AWS Secret Access Key:").grid(row=1, column=0, sticky=tk.W, pady=2)
        self.secret_key_var = tk.StringVar()
        secret_key_entry = ttk.Entry(self.access_key_frame, textvariable=self.secret_key_var, width=50, show="*")
        secret_key_entry.grid(row=1, column=1, sticky=tk.W, padx=(10, 0), pady=2)
        
        ttk.Label(self.access_key_frame, text="AWS Session Token (optional):").grid(row=2, column=0, sticky=tk.W, pady=2)
        self.session_token_var = tk.StringVar()
        session_token_entry = ttk.Entry(self.access_key_frame, textvariable=self.session_token_var, width=50, show="*")
        session_token_entry.grid(row=2, column=1, sticky=tk.W, padx=(10, 0), pady=2)
        
        # Profile Configuration Frame
        self.profile_frame = ttk.LabelFrame(scrollable_frame, text="AWS Profile Configuration", padding=10)
        self.profile_frame.pack(fill=tk.X, padx=10, pady=5)
        
        ttk.Label(self.profile_frame, text="Profile Name:").grid(row=0, column=0, sticky=tk.W, pady=2)
        self.profile_name_var = tk.StringVar()
        profile_entry = ttk.Entry(self.profile_frame, textvariable=self.profile_name_var, width=50)
        profile_entry.grid(row=0, column=1, sticky=tk.W, padx=(10, 0), pady=2)
        
        # List available profiles button
        list_profiles_btn = ttk.Button(self.profile_frame, text="List Available Profiles", 
                                     command=self.list_aws_profiles)
        list_profiles_btn.grid(row=1, column=1, sticky=tk.W, padx=(10, 0), pady=5)
        
        # SSO Configuration Frame
        self.sso_frame = ttk.LabelFrame(scrollable_frame, text="AWS SSO Configuration", padding=10)
        self.sso_frame.pack(fill=tk.X, padx=10, pady=5)
        
        ttk.Label(self.sso_frame, text="SSO Start URL:").grid(row=0, column=0, sticky=tk.W, pady=2)
        self.sso_start_url_var = tk.StringVar()
        sso_url_entry = ttk.Entry(self.sso_frame, textvariable=self.sso_start_url_var, width=50)
        sso_url_entry.grid(row=0, column=1, sticky=tk.W, padx=(10, 0), pady=2)
        
        ttk.Label(self.sso_frame, text="SSO Region:").grid(row=1, column=0, sticky=tk.W, pady=2)
        self.sso_region_var = tk.StringVar(value="us-east-1")
        sso_region_combo = ttk.Combobox(self.sso_frame, textvariable=self.sso_region_var, width=47)
        sso_region_combo['values'] = region_combo['values']
        sso_region_combo.grid(row=1, column=1, sticky=tk.W, padx=(10, 0), pady=2)
        
        ttk.Label(self.sso_frame, text="Account ID:").grid(row=2, column=0, sticky=tk.W, pady=2)
        self.sso_account_id_var = tk.StringVar()
        sso_account_entry = ttk.Entry(self.sso_frame, textvariable=self.sso_account_id_var, width=50)
        sso_account_entry.grid(row=2, column=1, sticky=tk.W, padx=(10, 0), pady=2)
        
        ttk.Label(self.sso_frame, text="Role Name:").grid(row=3, column=0, sticky=tk.W, pady=2)
        self.sso_role_name_var = tk.StringVar()
        sso_role_entry = ttk.Entry(self.sso_frame, textvariable=self.sso_role_name_var, width=50)
        sso_role_entry.grid(row=3, column=1, sticky=tk.W, padx=(10, 0), pady=2)
        
        # Role Assumption Configuration Frame
        self.role_frame = ttk.LabelFrame(scrollable_frame, text="IAM Role Assumption Configuration", padding=10)
        self.role_frame.pack(fill=tk.X, padx=10, pady=5)
        
        ttk.Label(self.role_frame, text="Role ARN:").grid(row=0, column=0, sticky=tk.W, pady=2)
        self.role_arn_var = tk.StringVar()
        role_arn_entry = ttk.Entry(self.role_frame, textvariable=self.role_arn_var, width=50)
        role_arn_entry.grid(row=0, column=1, sticky=tk.W, padx=(10, 0), pady=2)
        
        ttk.Label(self.role_frame, text="Session Name (optional):").grid(row=1, column=0, sticky=tk.W, pady=2)
        self.role_session_name_var = tk.StringVar()
        role_session_entry = ttk.Entry(self.role_frame, textvariable=self.role_session_name_var, width=50)
        role_session_entry.grid(row=1, column=1, sticky=tk.W, padx=(10, 0), pady=2)
        
        ttk.Label(self.role_frame, text="External ID (optional):").grid(row=2, column=0, sticky=tk.W, pady=2)
        self.external_id_var = tk.StringVar()
        external_id_entry = ttk.Entry(self.role_frame, textvariable=self.external_id_var, width=50)
        external_id_entry.grid(row=2, column=1, sticky=tk.W, padx=(10, 0), pady=2)
        
        ttk.Label(self.role_frame, text="MFA Serial (optional):").grid(row=3, column=0, sticky=tk.W, pady=2)
        self.mfa_serial_var = tk.StringVar()
        mfa_serial_entry = ttk.Entry(self.role_frame, textvariable=self.mfa_serial_var, width=50)
        mfa_serial_entry.grid(row=3, column=1, sticky=tk.W, padx=(10, 0), pady=2)
        
        ttk.Label(self.role_frame, text="MFA Token (optional):").grid(row=4, column=0, sticky=tk.W, pady=2)
        self.mfa_token_var = tk.StringVar()
        mfa_token_entry = ttk.Entry(self.role_frame, textvariable=self.mfa_token_var, width=50)
        mfa_token_entry.grid(row=4, column=1, sticky=tk.W, padx=(10, 0), pady=2)
        
        # Environment Variables Info Frame
        self.env_frame = ttk.LabelFrame(scrollable_frame, text="Environment Variables Information", padding=10)
...
• AWS_PROFILE (optional)"""
        ...
        self.default_frame = ttk.LabelFrame(scrollable_frame, text="Default Credential Chain Information", padding=10)
...
6. IAM roles for Lambda functions"""
        ...
        default_info_label = ttk.Label(self.default_frame, text=default_info_text, justify=tk.LEFT)
        ...
        connect_btn = ttk.Button(connection_frame, text="Connect to AWS", command=self.connect_to_aws)
        connect_btn.pack(side=tk.LEFT, padx=(0, 10))
        
        self.connection_status_var = tk.StringVar(value="Not Connected")
        status_label = ttk.Label(connection_frame, textvariable=self.connection_status_var, foreground="red")
    ...
    def setup_upload_tab(self):
        ...
        file_frame = ttk.LabelFrame(self.upload_frame, text="CSV File Selection", padding=10)
        ...
        # CSV format info
        info_frame = ttk.LabelFrame(self.upload_frame, text="CSV Format Information", padding=10)
...
Valid tiers: Standard, Advanced, Intelligent-Tiering"""
        ...
        # Upload controls
        upload_frame = ttk.LabelFrame(self.upload_frame, text="Upload Parameters", padding=10)
        upload_frame.pack(fill=tk.X, padx=10, pady=5)
        
        self.overwrite_var = tk.BooleanVar()
        overwrite_check = ttk.Checkbutton(upload_frame, text="Overwrite existing parameters without asking", variable=self.overwrite_var)
        overwrite_check.pack(anchor=tk.W, pady=5)
        
        upload_btn = ttk.Button(upload_frame, text="Upload Parameters", command=self.upload_parameters)
    ...
    def setup_view_tab(self):
        ...
        refresh_btn = ttk.Button(controls_frame, text="Refresh Parameters", command=self.refresh_parameters)
        refresh_btn.pack(side=tk.LEFT, padx=(0, 10))
        
        export_btn = ttk.Button(controls_frame, text="Export to CSV", command=self.export_parameters)
        export_btn.pack(side=tk.LEFT, padx=(0, 10))
        
        self.decrypt_var = tk.BooleanVar(value=True)
        decrypt_check = ttk.Checkbutton(controls_frame, text="Decrypt SecureString values", variable=self.decrypt_var)
        ...
        self.tree['columns'] = ('Value', 'Type', 'Tier', 'KeyId', 'LastModified', 'Version')
        ...
        self.context_menu.add_command(label="Copy Name", command=self.copy_parameter_name)
        self.context_menu.add_command(label="Copy Value", command=self.copy_parameter_value)
        self.context_menu.add_separator()
        self.context_menu.add_command(label="Delete Parameter", command=self.delete_selected_parameter)
    ...
    def list_aws_profiles(self):
        ...
        try:
            ...
            if profiles:
                ...
                messagebox.showinfo("Available AWS Profiles", f"Found profiles:\\n\\n{profile_list}")
            else:
                messagebox.showinfo("No Profiles Found", "No AWS profiles found. Please configure AWS CLI first.")
        ...
        try:
            ...
            if method == AuthMethod.ACCESS_KEY:
                if not self.access_key_var.get() or not self.secret_key_var.get():
                    messagebox.showerror("Error", "Please provide AWS Access Key ID and Secret Access Key")
                    return
                
                aws_config.set_access_key_auth(
                    access_key_id=self.access_key_var.get(),
                    secret_access_key=self.secret_key_var.get(),
                    session_token=self.session_token_var.get() if self.session_token_var.get() else None,
                ...
                if not all([self.sso_start_url_var.get(), self.sso_region_var.get(), 
                           self.sso_account_id_var.get(), self.sso_role_name_var.get()]):
                    messagebox.showerror("Error", "Please provide all SSO configuration details")
                    return
                ...
                aws_config.set_role_auth(
                    ...
                    role_session_name=self.role_session_name_var.get() if self.role_session_name_var.get() else None,
                    external_id=self.external_id_var.get() if self.external_id_var.get() else None,
                    mfa_serial=self.mfa_serial_var.get() if self.mfa_serial_var.get() else None,
                    mfa_token=self.mfa_token_var.get() if self.mfa_token_var.get() else None,
                    region=self.region_var.get()
                )
            ...
            def connect_thread():
                try:
                    if self.manager.connect():
                        ...
                        status_text = f"Connected ({auth_info['method']} - {auth_info['region']})"
                        
                        self.root.after(0, lambda: self.connection_status_var.set(status_text))
                        self.root.after(0, lambda: self.status_var.set("Connected to AWS"))
            ...
            messagebox.showerror("Configuration Error", f"Error configuring authentication: {str(e)}")
    ...
    def upload_parameters(self):
        ...
        def upload_thread():
            try:
                ...
                if not parameters:
                    self.root.after(0, lambda: messagebox.showwarning("Warning", "No valid parameters found in CSV file"))
                    return
                
                self.root.after(0, lambda: self.upload_log.delete('1.0', tk.END))
                self.root.after(0, lambda: self.log_message(f"Found {len(parameters)} parameters to upload\\n"))
                
                uploaded_count = 0
                skipped_count = 0
                
                for i, param in enumerate(parameters):
                    self.root.after(0, lambda p=param: self.status_var.set(f"Processing {p['key']}..."))
                    
                    # Check if parameter exists
                    exists, existing_param = self.manager.parameter_exists(param['key'])
                    
                    if exists and not self.overwrite_var.get():
                        self.root.after(0, lambda p=param, ep=existing_param: self.log_message(
                        ...
                        if result is None:  # Cancel
                            self.root.after(0, lambda: self.log_message("Upload cancelled by user\\n"))
                            break
                        elif not result:  # No
                            skipped_count += 1
                            self.root.after(0, lambda p=param: self.log_message(f"Skipped {p['key']}\\n"))
                            continue
                    ...
                    if self.manager.create_or_update_parameter(param, overwrite=True):
                        uploaded_count += 1
                        self.root.after(0, lambda p=param: self.log_message(f"✓ Uploaded {p['key']}\\n"))
                    else:
                        self.root.after(0, lambda p=param: self.log_message(f"✗ Failed to upload {p['key']}\\n"))
    ...
    def refresh_parameters(self):
        ...
        def refresh_thread():
            try:
                self.root.after(0, lambda: self.status_var.set("Refreshing parameters..."))
                parameters = self.manager.get_all_parameters(decrypt=self.decrypt_var.get())
    ...
    def populate_tree(self, parameters):
        ...
        for param in parameters:
            ...
            self.tree.insert('', tk.END, text=param['Name'], values=(
                display_value,
                param['Type'],
                param.get('Tier', ''),
                param.get('KeyId', ''),
    ...
    def filter_parameters(self, *args):
        ...
        filtered_params = [
            param for param in self.current_parameters
            if search_text in param['Name'].lower() or search_text in param['Value'].lower()
        ]
    ...
    def copy_parameter_value(self):
        ...
        if selected:
            ...
            param = next((p for p in self.current_parameters if p['Name'] == name), None)
            if param:
                self.root.clipboard_clear()
                ...
                def delete_thread():
                    try:
                        if self.manager.delete_parameter(name):
                            ...
                            self.root.after(0, lambda: self.status_var.set(f"Deleted parameter: {name}"))
                            # Remove from current_parameters list
                            self.current_parameters = [p for p in self.current_parameters if p['Name'] != name]
                        else:
    ...
    def export_parameters(self):
        ...
        if not self.current_parameters:
            messagebox.showwarning("Warning", "No parameters to export. Please refresh first.")
            return
        ...
        if filename:
            if self.manager.export_parameters_to_csv(filename, self.current_parameters):
                ...
                self.status_var.set(f"Exported {len(self.current_parameters)} parameters to CSV")
            else:
